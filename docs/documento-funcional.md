# Documento funcional — Dicsys Agent Suite
...
información de gestión de tres dominios de negocio — ventas/proyectos, documentación
interna y personal — y reciba una respuesta única, fundada en datos y con fuentes
...
| Actor | Descripción |
|---|---|
| Usuario de negocio | Dirección, líderes de proyecto, comercial, RRHH: hace consultas por CLI (o, a futuro, chat/portal) |
...
| Especialistas | Analista de Datos, Gestor Documental, Gestor de Personal |
...
|---|---|---|
| RF-01 | Recibir consultas en lenguaje natural por CLI (`dicsys-agents ask "..."`) | ✅ |
...
| RNF-05 | Calidad verificable en CI | 16 tests + lint + formato + smoke test en cada push |
| RNF-06 | Idioma: interacción y documentación en español | Prompts, CLI, docs |

### 3.3 Fuera de alcance (versión demo)

- Deploy productivo, interfaz web/chat, autenticación de usuarios.
...
- **Disparador:** `dicsys-agents ask "¿Cuánto facturamos este año y quiénes son nuestros principales clientes?"`
...
- **Disparador:** `dicsys-agents ask "¿Cuántos días de vacaciones le corresponden a alguien con 7 años de antigüedad?"`
...
- **Disparador:** `dicsys-agents ask "Necesito armar un equipo con Python: ¿qué perfiles tienen disponibilidad?"`
...
- **Disparador:** `dicsys-agents ask "¿Qué proyectos están en riesgo por consumo de horas?"`
...
- **Disparador:** `dicsys-agents ask --live "¿Cuánto facturamos a Banco Andino y quién puede tomar su próximo proyecto?"`
...
- **Disparador:** `dicsys-agents ask "¿Va a llover mañana?"`
...
|---|---|---|
| 1 | `python -m pytest` | 16 tests OK, < 5 s, sin red |
...
|---|---|---|
| 3 | `dicsys-agents demo` | Corre los 4 escenarios (CU-01 a CU-04) y muestra los resultados esperados de cada uno |
| 4 | `dicsys-agents -v ask "¿Qué proyectos están en riesgo?"` | La traza muestra `[orquestador] herramienta delegar_analista_datos` y `[analista_datos] herramienta avance_proyectos`; la respuesta marca P-2026-05 |
| 5 | `dicsys-agents ask "¿Va a llover mañana?"` | Respuesta controlada de fuera de dominio (CU-06) |
...
| 7 | `dicsys-agents ask --live "¿Cuántos días de vacaciones me corresponden con 7 años de antigüedad?"` | Respuesta "21 días hábiles" citando `politica-vacaciones.md` (CU-02 completo) |
| 8 | `dicsys-agents -v ask --live "¿Cuánto facturamos a Banco Andino y quién puede tomar su próximo proyecto?"` | La traza muestra delegación a **dos** especialistas; la síntesis integra facturación (150.700 USD) y perfiles disponibles (CU-05) |
